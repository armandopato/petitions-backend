...
export class ResolutionsService
{
    ...
    async createAssociatedResolution(petitionId: number, supportUser?: SupportTeamUser): Promise<Resolution>
    {
        ...
        if (await this.petitionsRepository.getPetitionStatus(petitionId) === PetitionStatus.TERMINATED) throw new ConflictException();
    ...
    async triggerTerminatedResolutionNotifications(resolution: Resolution): Promise<void>
    {
        return;
    }

    async triggerOverdueResolutionNotifications(resolution: Resolution): Promise<void>
    {
        return;
    }
}
