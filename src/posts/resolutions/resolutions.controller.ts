...
import { ResolutionTextDto } from './dto/resolution-text.dto';
...
export class ResolutionsController extends CommentsController<ResolutionComment>
{
    ...
    @Post()
    async createTerminated(@Request() req: AuthRequest<SupportTeamUser>,
                           @Body() createTerminatedResolutionDto: CreateTerminatedResolutionDto): Promise<{ resolutionId: number }>
    {
        return {
            resolutionId: await this.resolutionsService.createTerminated(createTerminatedResolutionDto, req.user),
        };
    }
    ...
    {
        return await this.resolutionsService.getSavedInfoPage(req.user, page);
    }
    ...
    @Put(':id')
    async terminateById(@Request() req: AuthRequest<SupportTeamUser>,
                        @Param('id', PositiveIntPipe) resolutionId: number,
                        @Body() resolutionTextDto: ResolutionTextDto): Promise<void>
    {
        await this.resolutionsService.terminateById(resolutionId, req.user, resolutionTextDto.resolutionText);
    }
    ...
    {
        return await this.resolutionsService.toggleSavedById(resolutionId, req.user);
    }
    ...
    async voteById(@Request() req: AuthRequest<StudentUser>,
                   @Param('id', PositiveIntPipe) resolutionId: number): Promise<void>
    {
        return await this.resolutionsService.voteById(resolutionId, req.user);
    }
}
